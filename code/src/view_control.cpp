...
glm::mat4 ViewControl::getViewMatrix() {
    // glm::vec3 g = m_gazeDir - m_eyePos;
    // glm::vec3 t = m_viewUp  - m_eyePos;
    // glm::vec3 w = - g / glm::length(g);
    // glm::vec3 tmp = glm::cross(t, w);
    // glm::vec3 u = tmp / glm::length(tmp);
    // glm::vec3 v = glm::cross(w, u);

    // glm::mat4 coordinate = glm::transpose(glm::mat4(
...
glm::mat4 ViewControl::getOrthoProjMatrix() {
    ...
    // return {
    //     glm::vec4(2.f / (m_r - m_l), 0.f, 0.f, 0.f),
    //     glm::vec4(0.f, 2.f / (m_t - m_b), 0.f, 0.f),
    //     glm::vec4(0.f, 0.f, 2.f / (m_f - m_n), 0.f),
    //     glm::vec4(-(m_r + m_l) / (m_r - m_l), -(m_t + m_b) / (m_t - m_b), -(m_f + m_n) / (m_f - m_n), 1.f)
    // };
}
